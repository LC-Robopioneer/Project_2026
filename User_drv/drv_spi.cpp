...
uint8_t SPI_Send_Receive_Data(SPI_HandleTypeDef *hspi, GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, uint16_t Tx_Length, uint16_t Rx_Length)
{
    ...
    if (hspi->Instance == SPI1)
    {
        ...
        return (HAL_SPI_TransmitReceive(hspi, SPI1_Manage_Object.Tx_Buffer, SPI1_Manage_Object.Rx_Buffer, Tx_Length, 100));
    ...
    else if (hspi->Instance == SPI2)
    {
        ...
        return (HAL_SPI_TransmitReceive(hspi, SPI2_Manage_Object.Tx_Buffer, SPI2_Manage_Object.Rx_Buffer, Tx_Length, 100));
    }
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
    ...
    if (hspi->Instance == SPI1)
    {
        //if (HAL_GPIO_ReadPin(SPI1_Manage_Object.Now_GPIOx, SPI1_Manage_Object.Now_GPIO_Pin) == GPIO_PIN_RESET)
        {
    ...
    {
        if (HAL_GPIO_ReadPin(SPI2_Manage_Object.Now_GPIOx, SPI2_Manage_Object.Now_GPIO_Pin) == GPIO_PIN_RESET)
        {
