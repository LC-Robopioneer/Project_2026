...
static HAL_StatusTypeDef SPI_Transmit_Receive(SPI_HandleTypeDef *hspi, uint8_t *tx_data, uint8_t *rx_data, uint16_t size)
{
    ...
    {
        memcpy(SPI1_Manage_Object.Tx_Buffer, tx_data, size);
        SPI_Send_Receive_Data(hspi, SPI1_Manage_Object.Now_GPIOx, SPI1_Manage_Object.Now_GPIO_Pin, size, SPI_BUFFER_SIZE);
    }
    else if (hspi->Instance == SPI2)
    {
        memcpy(SPI2_Manage_Object.Tx_Buffer, tx_data, size);
        SPI_Send_Receive_Data(hspi, SPI2_Manage_Object.Now_GPIOx, SPI2_Manage_Object.Now_GPIO_Pin, size, SPI_BUFFER_SIZE);
    }
    return HAL_OK;
}
...
{
    hspi = _hspi;
    cs_gpio_port = _cs_gpio_port;
    cs_gpio_pin = _cs_gpio_pin;
    status = IDLE;

    // 初始化片选引脚为高电平（不选中）
    //SPI_Init(hspi, SPI_Callback); // 这里可以传入回调函数指针，如果需要使用DMA或中断
    Write_Pin(cs_gpio_port, cs_gpio_pin, GPIO_PIN_SET);
...
uint8_t Class_MT6816::SPI_Transfer(uint8_t *tx_data, uint8_t *rx_data, uint16_t size)
{
    ...
    status = BUSY;
    return SPI_Transmit_Receive(hspi, tx_data, rx_data, size);
}
...
uint8_t Class_MT6816::ReadRegister(uint8_t reg_addr, uint8_t *data)
{
    ...
    // 准备发送数据
    tx_buffer_[0] = read_cmd; // 发送读取命令
    tx_buffer_[1] = 0x00;     // 发送dummy字节，用于接收数据
    tx_buffer_[2] = 0x00;     // 可选：第二个dummy字节

    // 发送读取命令并接收数据
    if (SPI_Transfer(tx_buffer_, rx_buffer_, 2) == HAL_OK)
    {
        *data = rx_buffer_[1]; // 第二个字节是读取的数据
        status = 0;
    }
...
uint8_t Class_MT6816::WriteRegister(uint8_t reg_addr, uint8_t data)
{
    ...
    // 准备发送数据
    tx_buffer_[0] = write_cmd; // 发送写入命令
    tx_buffer_[1] = data;      // 发送要写入的数据
